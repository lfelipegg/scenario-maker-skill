---
name: scenario-maker
description: Use when creating, refining, critiquing, or batching prompts for AI image and video generation, including character profiles, text-to-image, text-to-video, image-to-video, ComfyUI wildcards, prompt variants, tag prompts, Danbooru tags, negative prompts, Wan video prompts, and model-aware prompts for SDXL, Krea 2, Illustrious, NoobAI, or Pony.
---
...
- Read `references/model-prompts.md` and the selected file under `references/models/` when the request names SDXL, local Krea 2 RAW/Turbo, Illustrious/ILXL, NoobAI, Pony/Pony Diffusion, or another alias listed in the index. Load every named profile for a comparison.
...
- Length: Very Short unless the selected profile supplies another default (local Krea 2 uses Medium) or the user specifies a length or hard limit. Detail never authorizes invention or padding.
...
- Model coverage: use generic image/video guidance unless the user names Wan or a supported image model. Use Wan rules for Wan video prompts. Use `references/model-prompts.md` and its selected profile for SDXL, local Krea 2, Illustrious, NoobAI, or Pony image prompts. Do not guess a model when none is named.
