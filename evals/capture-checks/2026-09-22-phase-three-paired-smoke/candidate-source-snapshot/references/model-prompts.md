...
Use this reference only when the user explicitly names a supported image model or a clear alias. Read `routing.md`, `output-contracts.md`, [Scene Composition](scene-composition.md), and [Constraints and Revisions](constraints-and-revisions.md) first. Otherwise use generic task guidance without claiming model-specific adaptation.
...
`output-contracts.md` owns response shape and precedence. The profiles below supply only compatible, unspecified presentation defaults. They govern dialect, ordering, normalization, and model-facing syntax—not scene planning or permission to add facts. Apply them only after the operation, mode, scope, and protected content are resolved. A listed field, prefix, ordering slot, or example never requires absent subject, pose, action, setting, prop, camera, lighting, atmosphere, or style content.

Explicit prose, tag style, normalization, and prefix exclusions override these presentation preferences; they are not hard model input requirements. For a single prompt-writing request, return only the finished prompt. Negative-prompt sections apply only when a negative is requested. Model-note sections supply information only for requested notes or relevant analysis, never unsolicited additions. Preserve supplied facts and necessary implications, exact lettering, relationships, counts, exclusions, length, and safety constraints.

Every example below demonstrates presentation shape only. Do not copy or infer its scene facts when adapting another prompt.
...
- Express the authorized content as a concise natural-language description, usually 1-3 sentences.
- When present, place the main subject, action, setting, composition, lighting, and style in clear prose; do not fill missing categories.
- Prefer concrete visual terms over long tag stacks.
- Include camera/framing terms such as `close-up`, `wide shot`, `low angle`, `shallow depth of field`, or `cinematic lighting` only when supplied or permitted.
...
- Start with any applicable score tags, then source/rating, then the authorized subject and visual tags.
- Default safe prefix: `score_9, score_8_up, score_7_up, source_anime, rating_safe`.
- Choose one source tag that matches the supplied content: `source_anime`, `source_cartoon`, `source_pony`, `source_furry`, or another source tag the user provides.
- Use `rating_safe` by default unless the user requests another allowed rating.
- Use comma-separated tags and short visual phrases for the content already supplied or permitted.
...
- Start with quality/style guidance, then order whichever authorized fields are present: subject count, character traits, pose/action, setting, lighting, and composition.
...
- When composition is supplied or permitted, keep its tags restrained: `close-up`, `upper_body`, `full_body`, `cowboy_shot`, `looking_at_viewer`, `dynamic_angle`.
- Natural-language fragments are acceptable only when a protected detail has no clean tag equivalent.
...
- Start with quality/model preference tags, then order whichever authorized fields are present: subject, identity/source, traits, action, setting, lighting, and composition.
...
- Keep authorized content visible and concrete. Do not add lore, relationships, hidden personality traits, or any visual field merely to complete the ordering.
- Do not use Pony score/source/rating tags unless the user explicitly asks for Pony.
...
When creating wildcard files for a named supported model, apply the requested format and authorized content first. Otherwise use these compatible presentation defaults:

- Each full-prompt line renders the selected content in the selected model's default dialect.
...
- SDXL lines should use compact natural-language phrasing.
