...
For one prompt-writing request, return the finished prompt only. Do not add headings, wrappers, notes, explanations, negative prompts, alternatives, or model metadata unless requested. The prompt itself may contain syntax required by its resolved model, task, or interface.
...
Ordinary Danbooru Version uses canonical tags where possible. First express required concepts through faithful tags without broadening or narrowing their meaning. If a required concept has no faithful canonical expression, use only the minimal literal qualifier or relational phrase needed to preserve it; do not repeat a subject or attribute already expressed by the surrounding tags. A fallback is not a license to replace the requested tag style with general prose.
