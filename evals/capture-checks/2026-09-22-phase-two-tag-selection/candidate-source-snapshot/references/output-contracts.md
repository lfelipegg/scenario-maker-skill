...
Use a literal qualifier or relational phrase only for required meaning that the tag combination cannot express. Keep only the missing meaning: do not repeat an already-tagged subject or attribute, and prefer an equally faithful shorter qualifier over a redundant noun phrase. Do not use the fallback for optional quality boilerplate, convenience, or avoiding a known tag conversion. Explicit lettering and relationships retain the full wording needed for fidelity.
