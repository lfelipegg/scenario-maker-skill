...
**H3-only enhancement offer:** after an H3 prompt, a brief optional in-chat enhancement offer may appear outside the pasteable prompt when the requested output permits it. An explicit “prompt only,” equivalent restriction, machine-readable-only output, or artifact-only file content suppresses it. Do not add a second prompt, notes package, or automatic enhancement. Other targets retain the prompt-only default. Enhancement happens only after a request, under unchanged permissions and locks, without invoking an external service.
