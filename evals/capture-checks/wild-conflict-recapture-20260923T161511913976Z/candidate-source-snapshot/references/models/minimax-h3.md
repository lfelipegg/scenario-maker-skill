...
- Default video intent is one shot when no relevant delegation exists. Preserve explicitly requested cuts, shot order, actor/action bindings, overlap, timing, pace, and audio continuity. Wild video development may choose cuts; those cuts are authorized rather than padding. Never invent cut times or timestamps, runtime, frame count, or frame-accurate scheduling.
- Leave camera behavior unspecified when neither the user nor valid Wild video delegation chooses it. Under Wild, camera behavior may be selected to serve the concept. Preserve requested movement or fixed framing and all supplied timeline or shot restrictions; clarify genuinely incompatible camera or framing locks rather than weakening them.
...
Assign only named roles: identity, appearance, clothing, composition, motion, camera, style, source-video editing, audio reuse, or audio reference. An appearance source does not donate its background, pose, dialogue, or soundtrack. Wild never invents an input asset, supplies an unknown reference role, or copies unassigned properties; it may create authorized new target content without pretending that content came from a reference. Clarify material role conflicts rather than choosing silently.
...
- Establish each vocal source from supplied or inspectable distinguishing traits and permitted vocal properties. Wild may author concise dialogue or lyrics and choose concept-serving vocal qualities; otherwise never invent age, accent, pitch, timbre, or a visible narrator to fill the structure.
- Use `<d>[Language] exact words</d>` for supplied and Wild-authored speech or lyrics. Preserve supplied text, language, spelling, and punctuation, including repeated punctuation, unless revision or translation was authorized. For Wild-authored words, use an explicitly requested language or the profile's English default. Keep speaker identity, action, delivery, and reference-role prose outside `<d>`. Use `[unclear]` only for genuinely unintelligible source spans. A timbre-only reference does not authorize source dialogue.
...
- For requested or Wild-authorized cuts without supplied times, use sequential `[Shot N]` labels and preserve the intended relative order and overlap without inventing timestamps or asking for optional timing. An explicit no-timestamp format remains valid.
- When one supplied or Wild-authored line crosses a cut, put `<scenetrans>` at the connecting end and start of the two dialogue fragments and state that the same audio continues uninterrupted. Do not repeat the full line, reword its fragments, or restart the speaker. The marker is structure, not an extra spoken word.
...
- Put supplied or Wild-authorized camera motion in natural prose in the relevant shot. Preserve its motion type and any supplied amplitude or speed; do not require missing amplitude or speed. Distinguish optical zoom from moving the camera forward, and pan from sideways camera translation. Do not replace a requested cut with a camera move merely because an upstream example recommends it.
- For Ref2VA, put supplied or Wild-authorized overall style before `[Shot 1]`. For I2VA, establish source-derived style and composition inside `[Shot 1]`. If style is unspecified and not authorized by Wild or established by permitted evidence, omit it.
- Prefer concrete supplied or Wild-authorized visual and audible details over praise or generic cinematic filler. Wild may select concept-serving style, instrumentation, tempo, dynamics, camera, and sounds without requiring every field. Do not delete or replace supplied intent, invent concrete instruments to reinterpret a supplied mood, or expand sparse non-Wild sources to hit a word target.

### Sound Placement

- The timeline field owns supplied or Wild-authorized speech, singing, in-scene music such as radio, phone, or television audio, and precisely synchronized sound events.
- `overall_soundscape` summarizes supplied or Wild-authorized ambience, physical sounds, and non-verbal human sounds across the clip. Do not duplicate full dialogue or lyrics or classify diegetic songs or background score here.
- `non_diegetic_music` describes supplied or Wild-authorized audience-only score using permitted instrumentation, tempo, and dynamics. Copy or reference relationships belong only in the section for the affected audio layer; a mixed asset may be cited in both sections for different assigned layers.
...
Use role-bound images, videos, and audio to synthesize a target video. After binding every reference role, communicate only supplied or authorized subjects, relationships, appearance, setting, composition, motion, camera, style, and sound. Wild may develop new target content but identity-only sources still do not donate backgrounds, action, audio, or other unassigned properties. Preserve rosters, exclusions, locked details, and exact lettering. Transfer only the named properties from each supplemental reference.
...
Identify the source video being edited, every authorized change, and the content to preserve. Wild does not reopen unaffected dimensions of a source-video edit. Do not silently reuse, replace, remove, or invent its soundtrack. Clear edits apply locally; unaffected protected content remains protected.
...
Explicitly requested or Wild-authorized multi-shot intent is allowed: use sequential shot labels and preserve supplied cuts, order, overlap, timing, and audio continuity without inventing timestamps, runtime, or scheduler precision.
