...
Use this reference only when the user explicitly names a supported image model or a clear alias. Otherwise use the generic prompt rules in `prompt-types.md`.
...
If multiple supported models are named, produce separate prompt packages for each unless the user asks for a single merged format.

## Default Package Shape

For supported model requests, return:

1. Positive prompt
2. Negative prompt, when useful
3. Model notes, only compact actionable details

Model profiles override the generic Normal, Tag, and Danbooru defaults only for the current model-targeted request. Preserve user-specified content, length, exclusions, and safety constraints.
...
- Keep notes brief; Pony's required tag dialect is the important part.
...
When creating wildcard files for a named supported model:

- Each full-prompt line must use the selected model's dialect.
